...
def em_algorithm(sequences, max_iter=100, tol=1e-6):
    ...
    P_A = 0.5  # Coin A'nın seçilme olasılığı (prior)

    for iteration in range(max_iter):
        Q_A_values = []
        Q_B_values = []

        # E Adımı: Q_A ve Q_B değerlerini hesapla
        for sequence in sequences:
            ...
            likelihood_B = ( comb(trials, heads) * (Q_B ** heads) * ((1 - Q_B) ** tails) ) * (1 - P_A)
        ...
        # M Adımı: Q_A, Q_B ve P_A değerlerini güncelle
        ...
        new_P_A = sum(Q_A_values) / len(sequences)  # Coin A'nın prior olasılığı

        # Durma kriteri
        if abs(new_Q_A - Q_A) < tol and abs(new_Q_B - Q_B) < tol and abs(new_P_A - P_A) < tol:
            break

        # Güncellenen parametreleri ata
        Q_A, Q_B, P_A = new_Q_A, new_Q_B, new_P_A

    return Q_A, Q_B, P_A, Q_A_values, Q_B_values
